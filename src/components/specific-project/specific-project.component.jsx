...
import { DB_BASE_URL } from "../../dbconnection/connection";
...
const SProject = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      const fetch = await axios.get(`${DB_BASE_URL}/${machineLearningId}/`);
  ...
  const handleClick = async () => {
    ...
    let fetch_results = "";
    try {
      fetch_results = await axios.post(
        ` https://machinelearningpred.herokuapp.com/predict_${data.dir}`,
        {
  ...
  return (
    <div className="specific-project" onClick={outsideClick}>
      ...
      <fieldset className="parametersField">
        ...
        {results ? (
          <AIResult response={responseResults} results={results} />
        ) : (
          ""
        )}
      </fieldset>
